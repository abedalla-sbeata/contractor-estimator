...
type Step = "form" | "chat" | "success";

type EstimateFlowProps = {
  /** Taken from the URL (/c/CTR-XXXXX) — never entered by the client. */
  contractorCode: string;
};

function assistantFailureText(next: Estimate): string | null {
  return next.error_message || next.assistant_message || null;
}

export function EstimateFlow({ contractorCode }: EstimateFlowProps) {
  ...
  }, [messages, step, pendingFiles.length, thinking, estimate?.status]);
  ...
  // Poll while the report is being prepared (status === processing).
  useEffect(() => {
    if (!estimate || step !== "chat") return;
    if (estimate.status !== "processing") return;
    ...
    async function pollOnce() {
      try {
        const next = await api.getEstimate(estimateId);
        if (cancelled) return;
        ...
        if (next.status === "failed") {
          const failText = assistantFailureText(next);
          if (failText) {
            setMessages((prev) => {
              const last = prev[prev.length - 1];
              if (last?.role === "assistant" && last.content === failText) {
                return prev;
              }
              return [
                ...prev,
          ...
          setError(failText || t("estimate.failedBody"));
        }
      } catch {
  ...
  }, [estimate?.id, estimate?.status, step, t]);
  ...
  /** Drive UI from API status only. Chat stays open for collecting / processing / failed. */
  function applyStatus(next: Estimate) {
    setEstimate(next);
  ...
  function finishWithEstimate(next: Estimate, appendAssistant = true) {
    if (appendAssistant && next.assistant_message) {
      // Prefer error_message in the bubble when failed; assistant_message may duplicate it.
      const content =
        next.status === "failed"
          ? assistantFailureText(next) || next.assistant_message
          : next.assistant_message;
      appendAssistantMessage(content);
    } else if (next.status === "failed" && next.error_message) {
      appendAssistantMessage(next.error_message);
    }
  ...
  async function onStart(e: FormEvent) {
    ...
    try {
      const result = await api.startEstimate({
        contractor_code: normalizedCode,
  ...
  async function onSend(e: FormEvent) {
    ...
    // Only collecting / failed accept messages; processing locks input.
    if (estimate.status === "processing" || estimate.status === "sent") return;
    ...
    try {
      ...
      // Keep chatting while collecting; backend decides when to prepare the report.
  ...
  if (step === "chat" && estimate) {
    ...
    const inputLocked = processing || thinking;
      ...
      <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-3xl flex-col px-4 py-4 sm:px-6 sm:py-6">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-steel-200 bg-white shadow-sm">
          ...
          >
            ...
            <div
              className={`flex items-end gap-2 rounded-2xl border border-steel-200 bg-steel-50 px-2 py-2 focus-within:border-accent-500 focus-within:ring-2 focus-within:ring-accent-500/20 ${
            ...
            >
              ...
              <textarea
                ...
                placeholder={
                  processing
                    ? t("estimate.processing")
                    : t("estimate.chatPlaceholder")
                }
                rows={1}
                disabled={inputLocked}
